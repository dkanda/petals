...
*   **Goal**: Fix the prompt embeddings shape and indexing for deep p-tuning.
*   **Implementation**:
    *   [x] Change `config.num_hidden_layers` to `config.num_hidden_layers - 1` in `intermediate_prompt_embeddings` instantiation in `src/petals/client/ptune.py`.
    *   [x] Update the `.view()` shape in `get_prompt()` to use `config.num_hidden_layers - 1` and prepend zero-padding for the first layer in `src/petals/client/ptune.py`.
