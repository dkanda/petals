...
## Code Refactoring (High Priority) [DONE]
*   **Goal**: Fix intermediate prompt shapes for deep p-tuning
*   **Implementation**:
    *   [x] Fix shape of `intermediate_prompt_embeddings` to account for `num_hidden_layers - 1` in `src/petals/client/ptune.py`.
    *   [x] Fix shape of `.view()` in `get_prompt` to correctly reshape `intermediate_prompts` using `num_hidden_layers - 1` in `src/petals/client/ptune.py`.
