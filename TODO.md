...
*   **Goal**: Fix the off-by-one error in deep p-tuning layer count and align with `RemoteSequential` logic.
*   **Implementation**:
    *   [x] Fix `intermediate_prompt_embeddings` instantiation to use `num_hidden_layers - 1`.
    *   [x] Prepend a zero-padding tensor to `intermediate_prompts` in `get_prompt`.
