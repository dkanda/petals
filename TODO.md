...
*   **Goal**: Fix Deep P-Tuning intermediate prompts generation to properly align with `RemoteSequential` logic.
*   **Implementation**:
    *   [x] Update `intermediate_prompt_embeddings` to hold parameters for `num_hidden_layers - 1`.
    *   [x] Update `get_prompt` to prepend a zero-padded tensor for the first layer.
