...
## 8. Fix Deep PTune Shape (High Priority) [DONE]
*   **Goal**: Fix the shape logic of deep ptune to be compatible with model blocks.
*   **Implementation**:
    *   [x] Change intermediate prompt shape from `num_hidden_layers` to `num_hidden_layers - 1`.
    *   [x] Add zero padding for first layer prompt to make the output `(num_hidden_layers, batch_size, pre_seq_len, hidden_size)`.
    *   [x] Add unit test.
