...
*   **Goal**: Fix the prompt dimension sizes in Deep PTune mode when initializing prompts and calling `get_prompt()`.
*   **Implementation**:
    *   [x] Fix `intermediate_prompt_embeddings` initialization and reshaping to use `num_hidden_layers - 1`.
    *   [x] Add zero padding block for compatibility.
