...
*   **Goal**: Fix the prompt shapes in deep prompt tuning to correctly align with the hidden layers and prepend zero-padding.
*   **Implementation**:
    *   [x] Fix `intermediate_prompt_embeddings` initialization in `src/petals/client/ptune.py`.
    *   [x] Add unit testing for `deep_ptune` and `ptune` modes.
