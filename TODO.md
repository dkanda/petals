...
*   **Goal**: Fix the dimensions and initialization logic for Deep P-tuning as indicated by inline TODOs in `src/petals/client/ptune.py`.
*   **Implementation**:
    *   [x] Reduce `intermediate_prompt_embeddings` size to `num_hidden_layers - 1`.
    *   [x] Update `get_prompt` to correctly zero-pad the first layer prompt.
    *   [x] Write unit tests for P-tuning initialization and prompt generation.
