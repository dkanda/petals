...
*   **Goal**: Refactor `intermediate_prompt_embeddings` to align with the number of expected layers.
*   **Implementation**:
    *   [x] Update `intermediate_prompt_embeddings` parameter size to use `config.num_hidden_layers - 1`.
    *   [x] Prepend a zero-padding tensor for the first layer's prompt slice.
