...
*   **Goal**: Ensure `intermediate_prompt_embeddings` has the correct shape and prompts are prepended with zero-padding properly for `deep_ptune`.
*   **Implementation**:
    *   [x] Change `config.num_hidden_layers` to `config.num_hidden_layers - 1` for `intermediate_prompt_embeddings`.
    *   [x] Adjust `intermediate_prompts` view shape in `get_prompt` to `config.num_hidden_layers - 1`.
    *   [x] Prepend zero-padding tensor for the first layer in `get_prompt` so lengths match.
