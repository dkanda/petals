...
*   **Goal**: Optimize Deep P-Tuning embeddings memory usage.
*   **Implementation**:
    *   [x] Adjust `intermediate_prompt_embeddings` to allocate for `num_hidden_layers - 1` instead of `num_hidden_layers`.
    *   [x] Update `get_prompt` to prepend a zero-padding tensor for the first layer to maintain compatibility with `RemoteSequential` logic.
