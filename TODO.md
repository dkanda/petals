...
## 8. Deep PTune Fixes (High Priority) [DONE]
*   **Goal**: Fix shape mismatch issues in Deep PTune mode prompt embeddings.
*   **Implementation**:
    *   [x] Update `src/petals/client/ptune.py` to use `num_hidden_layers - 1` when calculating shapes.
    *   [x] Ensure `get_prompt` prepends a zero-padding tensor correctly.
