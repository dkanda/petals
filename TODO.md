...
*   **Goal**: Fix `# TODO: should be num_hidden_layers - 1` in `PTuneMixin`
*   **Implementation**:
    *   [x] Fix shape issues by replacing `config.num_hidden_layers` with `config.num_hidden_layers - 1` in `intermediate_prompt_embeddings` and `intermediate_prompts` view.
    *   [x] Add unit test to verify the shapes.
