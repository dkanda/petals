...
## 8. Fix deep ptune layer sizes (High Priority) [DONE]
*   **Goal**: Fix the prompt embeddings shape bug in deep P-tuning logic.
*   **Implementation**:
    *   [x] Fix `num_hidden_layers` in `PTuneMixin.init_prompts` and `PTuneMixin.get_prompt`.
    *   [x] Add deep ptune tests to prevent future regressions.
