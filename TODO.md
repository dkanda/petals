...
## 8. Deep P-tuning Memory Optimization (Medium Priority) [DONE]
*   **Goal**: Optimize `deep_ptune` by allocating parameters only for `num_hidden_layers - 1`.
*   **Implementation**:
    *   [x] Update `intermediate_prompt_embeddings` to use `num_hidden_layers - 1` in `init_prompts`.
    *   [x] Reshape intermediate prompts for `num_hidden_layers - 1` in `get_prompt`.
    *   [x] Prepend zero-padding for the first layer to maintain compatibility with `RemoteSequential`.
