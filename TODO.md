...
## 8. Fix deep_ptune shape issues (High Priority) [DONE]
*   **Goal**: Fix the `deep_ptune` implementation where `intermediate_prompt_embeddings` has the wrong shape.
*   **Implementation**:
    *   [x] Change intermediate embeddings to use `num_hidden_layers - 1` in `init_prompts`.
    *   [x] Update shape calculation in `get_prompt` to use `num_hidden_layers - 1`.
    *   [x] Prepend zero padding for the first layer in `get_prompt`.
