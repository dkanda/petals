...
## 8. Deep P-Tuning Fixes (Medium Priority) [DONE]
*   **Goal**: Fix the prompt embeddings shape and layer offsets in `PTuneMixin`.
*   **Implementation**:
    *   [x] Change `num_hidden_layers` to `num_hidden_layers - 1` in `init_prompts` and `get_prompt` in `src/petals/client/ptune.py`.
