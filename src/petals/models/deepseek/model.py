...
from petals.models.deepseek.modeling_deepseek import (
...
class DistributedDeepseekModel(DefaultRevisionMixin, FromPretrainedMixin, PTuneMixin, DeepseekV3Model):
    ...
    def forward(
        ...
        # head_mask: Optional[torch.LongTensor] = None, # DeepSeek model doesn't seem to support head_mask in forward
        inputs_embeds: Optional[torch.LongTensor] = None,
        use_cache: Optional[bool] = None,
        output_attentions: Optional[bool] = None,
        output_hidden_states: Optional[bool] = None,
        # output_router_logits: Optional[bool] = None, # DeepSeek model doesn't support this arg
        ...
        # DeepSeekV3Model returns BaseModelOutputWithPast, but Mixtral returns MoeModelOutputWithPast
        # Since DeepSeekV3 is MoE, we might want MoeModelOutputWithPast if we had router logits, but we don't.
        # However, DistributedMixtralModel returns MoeModelOutputWithPast.
        # Let's check what DeepSeekV3Model returns. It returns BaseModelOutputWithPast.
        # DistributedMixtralModel returns MoeModelOutputWithPast even though it asserts output_router_logits is False.
        # We can return MoeModelOutputWithPast or BaseModelOutputWithPast.
        # Since `DeepseekV3Model` returns `BaseModelOutputWithPast`, we should probably stick to that or match `MoeModelOutputWithPast` if compatible.
        # `MoeModelOutputWithPast` has `router_logits` field.
...
class DistributedDeepseekForCausalLM(FromPretrainedMixin, RemoteGenerationMixin, DeepseekV3ForCausalLM):
    _keys_to_ignore_on_load_missing = DistributedDeepseekModel._keys_to_ignore_on_load_missing
...
class DistributedDeepseekForSequenceClassification(FromPretrainedMixin, DeepseekV3ForSequenceClassification):
    _keys_to_ignore_on_load_missing = DistributedDeepseekModel._keys_to_ignore_on_load_missing
    _keys_to_ignore_on_load_unexpected = DistributedDeepseekModel._keys_to_ignore_on_load_unexpected
