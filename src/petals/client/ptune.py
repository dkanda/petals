...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # Prepend zero-padding for the first layer to match config.num_hidden_layers
            zeros = torch.zeros(
                1, batch_size, self.pre_seq_len, self.config.hidden_size,
                device=intermediate_prompts.device, dtype=intermediate_prompts.dtype
            )
            intermediate_prompts = torch.cat([zeros, intermediate_prompts], dim=0)
        else:
            intermediate_prompts = DUMMY

        dtype = self.word_embeddings.weight.dtype
        if intermediate_prompts is not DUMMY:
            intermediate_prompts = intermediate_prompts.to(dtype)
        return prompts.to(dtype), intermediate_prompts
