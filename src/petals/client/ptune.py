...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # The first layer prompt is provided by `prompts` above.
            # Downstream logic expects a shape of (num_hidden_layers, batch_size, pre_seq_len, hidden_size).
            # We prepend a dummy (zero) tensor for the first layer to match the expected shape.
            first_layer_dummy = torch.zeros(
                1, batch_size, self.pre_seq_len, self.config.hidden_size,
                device=intermediate_prompts.device, dtype=intermediate_prompts.dtype
            )
            intermediate_prompts = torch.cat([first_layer_dummy, intermediate_prompts], dim=0)
        else:
