...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # Pad intermediate_prompts with zeros for the first layer
            # (which uses the prompts returned separately)
            # Shape before: (num_hidden_layers - 1, batch_size, pre_seq_len, hidden_size)
            # Shape after: (num_hidden_layers, batch_size, pre_seq_len, hidden_size)
            padding = torch.zeros(
                1, batch_size, self.pre_seq_len, self.config.hidden_size,
                device=intermediate_prompts.device, dtype=intermediate_prompts.dtype
            )
            intermediate_prompts = torch.cat([padding, intermediate_prompts], dim=0)
        else:
