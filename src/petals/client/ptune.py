...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # Pad the first layer's prompt with zeros to align shape to (num_hidden_layers, batch_size, pre_seq_len, hidden_size)
            zero_pad = torch.zeros(
                1, batch_size, self.pre_seq_len, self.config.hidden_size, device=intermediate_prompts.device
            )
            intermediate_prompts = torch.cat([zero_pad, intermediate_prompts], dim=0)
        else:
