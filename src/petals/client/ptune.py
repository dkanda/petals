...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # We prepend a zero-padding tensor for the first layer to maintain
            # compatibility with RemoteSequential logic that expects prompts for all layers.
            layer_0_padding = torch.zeros(
                1, batch_size, self.pre_seq_len, self.config.hidden_size,
                device=intermediate_prompts.device, dtype=intermediate_prompts.dtype
            )
            intermediate_prompts = torch.cat([layer_0_padding, intermediate_prompts], dim=0)
        else:
