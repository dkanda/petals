...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # Prepend a zero-padding tensor for the first layer to match expected shape
            # The expected shape is (num_hidden_layers, batch_size, pre_seq_len, hidden_size)
            first_layer_padding = torch.zeros(
                1,
                batch_size,
                self.pre_seq_len,
                self.config.hidden_size,
            ...
            intermediate_prompts = torch.cat([first_layer_padding, intermediate_prompts], dim=0)
        else:
