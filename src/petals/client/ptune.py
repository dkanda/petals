...
class PTuneMixin:
    ...
    def get_prompt(self, batch_size):
        ...
        if self.config.tuning_mode == "deep_ptune":
            ...
            # Prepend zero-padding for the first layer to maintain compatibility with RemoteSequential
            first_layer_zeros = torch.zeros(
                1,
                batch_size,
                self.pre_seq_len,
                self.config.hidden_size,
            ...
            intermediate_prompts = torch.cat([first_layer_zeros, intermediate_prompts], dim=0)
        else:
