...
def main():
    ...
    group = basic_group.add_mutually_exclusive_group(required=True)
    ...
    group = basic_group.add_mutually_exclusive_group(required=False)
    ...
    basic_group.add_argument('--num_blocks', type=int, default=None, help="The number of blocks to serve")
    basic_group.add_argument('--block_indices', type=str, default=None, help="Specific block indices to serve")
    basic_group.add_argument('--dht_prefix', type=str, default=None, help="Announce all blocks with this DHT prefix")
    ...
    performance_group.add_argument('--compression', type=str, default='NONE', required=False, help='Tensor compression communication')

    performance_group.add_argument('--num_handlers', type=int, default=8, required=False,
                        help='server will use this many processes to handle incoming requests')
    performance_group.add_argument('--prefetch_batches', type=int, default=1, required=False,
                        help='Pre-form this many subsequent batches while GPU is processing the current one')
    performance_group.add_argument('--sender_threads', type=int, default=1, required=False,
                        help='Use this many threads to pass results/exceptions from Runtime to Pools')

    performance_group.add_argument('--inference_max_length', type=int, default=None,
                        help='Maximum total sequence length permitted per inference, defaults to 16384 tokens. '
                             'Default: 8192 for models with multi-query attention (based on Llama 2, Falcon), 2048 for others')
    performance_group.add_argument('--min_batch_size', type=int, default=1,
                        help='Minimum required batch size for all operations (in total tokens)')
    performance_group.add_argument('--max_batch_size', type=int, default=None,
                        help='The total number of tokens in the same batch will not exceed this value. '
                             'Default: 8192 for models with multi-query attention (based on Llama 2, Falcon), 2048 for others')
    performance_group.add_argument('--max_chunk_size_bytes', type=int, default=256 * 1024 * 1024,
                        help='Maximum size of activation tensor processed in one go; larger tensors are split into chunks')
    performance_group.add_argument('--attn_cache_tokens', type=int, default=None,
    ...
    hardware_group.add_argument('--cache_dir', type=str, default=None,
                        help='Path to a directory in which a downloaded pretrained model configuration should be cached if the standard cache should not be used.')
    hardware_group.add_argument("--max_disk_space", type=str, default=None,
    ...
    hardware_group.add_argument('--device', type=str, default=None, required=False,
                        help='all blocks will use this device in torch notation; default: cuda if available else cpu')
    hardware_group.add_argument("--torch_dtype", type=str, choices=DTYPE_MAP.keys(), default="auto",
                        help="Use this dtype to store block weights and do computations. "
                             "By default, respect the dtypes in the pre-trained state dict.")
    hardware_group.add_argument('--max_alloc_timeout', type=float, default=600,
                        help="If the cache is full, the server will wait for memory to be freed up to this many seconds"
                             " before rejecting the request")
    basic_group.add_argument('--revision', type=str, default=None,
                        help="The specific model version to use. It can be a branch name, a tag name, or a commit id, since we use a git-based system for storing models"
                             "and other artifacts on huggingface.co, so `revision` can be any identifier allowed by git.")

    performance_group.add_argument('--throughput',
    ...
    net_group.add_argument('--update_period', type=float, required=False, default=120,
                        help='Server will report blocks to DHT once in this many seconds')
    net_group.add_argument('--expiration', type=float, required=False, default=None,
                        help='DHT entries will expire after this many seconds')
    performance_group.add_argument('--request_timeout', type=float, required=False, default=3 * 60,
                        help='Timeout (in seconds) for the whole rpc_forward/rpc_backward/rpc_forward_stream/rpc_backward_stream request')
    performance_group.add_argument('--session_timeout', type=float, required=False, default=30 * 60,
                        help='Timeout (in seconds) for the whole inference session')
    performance_group.add_argument('--step_timeout', type=float, required=False, default=5 * 60,
                        help="Timeout (in seconds) for waiting the next step's inputs inside an inference session")

    group = net_group.add_mutually_exclusive_group()
    ...
    advanced_group.add_argument('--increase_file_limit', type=int, default=4096,
    ...
    advanced_group.add_argument('--stats_report_interval', type=int, required=False,
                        help='Interval between two reports of batch processing performance statistics')

    advanced_group.add_argument('--custom_module_path', type=str, required=False,
                        help='Path of a file with custom nn.modules, wrapped into special decorator')
    advanced_group.add_argument('--identity_path', type=str, required=False, help='Path to identity file to be used in P2P')

    performance_group.add_argument("--balance_quality", type=float, default=0.75,
    ...
    performance_group.add_argument("--mean_balance_check_period", type=float, default=60,
                        help="Check the swarm's balance every N seconds (and rebalance it if necessary)")

    hardware_group.add_argument('--quant_type', type=str, default=None, choices=[choice.name.lower() for choice in QuantType],
    ...
    hardware_group.add_argument("--tensor_parallel_devices", nargs='+', default=None,
                        help=
    ...
    basic_group.add_argument("--adapters", nargs='*', default=(),
