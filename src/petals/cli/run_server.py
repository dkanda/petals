...
def main():
    ...
    auth_group.add_argument("--token", type=str, default=None,
                        help="Hugging Face Hub auth token for attending private models. "
                             "Mutually exclusive with --use_huggingface_cli_token")
    auth_group.add_argument("--use_auth_token", action="store_true", dest="token",
                        help="Deprecated. Use --use_huggingface_cli_token instead")
    auth_group.add_argument("--use_huggingface_cli_token", action="store_true", dest="token",
    ...
    serving_group.add_argument('--dht_prefix', type=str, default=None, help="Announce all blocks with this DHT prefix")
    serving_group.add_argument("--adapters", nargs='*', default=(),
                        help="List of pre-loaded LoRA adapters that can be used for inference or training")
    serving_group.add_argument('--revision', type=str, default=None,
    ...
    perf_group.add_argument('--prefetch_batches', type=int, default=1, required=False,
                        help='Pre-form this many subsequent batches while GPU is processing the current one')
    perf_group.add_argument('--sender_threads', type=int, default=1, required=False,
                        help='Use this many threads to pass results/exceptions from Runtime to Pools')
    perf_group.add_argument('--inference_max_length', type=int, default=None,
                        help='Maximum total sequence length permitted per inference, defaults to 16384 tokens. '
                             'Default: 8192 for models with multi-query attention (based on Llama 2, Falcon), 2048 for others')
    perf_group.add_argument('--min_batch_size', type=int, default=1,
                        help='Minimum required batch size for all operations (in total tokens)')
    perf_group.add_argument('--max_batch_size', type=int, default=None,
                        help='The total number of tokens in the same batch will not exceed this value. '
                             'Default: 8192 for models with multi-query attention (based on Llama 2, Falcon), 2048 for others')
    perf_group.add_argument('--max_chunk_size_bytes', type=int, default=256 * 1024 * 1024,
                        help='Maximum size of activation tensor processed in one go; larger tensors are split into chunks')
    perf_group.add_argument('--attn_cache_tokens', type=int, default=None,
    ...
    perf_group.add_argument('--throughput',
    ...
    adv_group.add_argument('--expiration', type=float, required=False, default=None,
                        help='DHT entries will expire after this many seconds')
    adv_group.add_argument('--request_timeout', type=float, required=False, default=3 * 60,
                        help='Timeout (in seconds) for the whole rpc_forward/rpc_backward/rpc_forward_stream/rpc_backward_stream request')
    adv_group.add_argument('--session_timeout', type=float, required=False, default=30 * 60,
                        help='Timeout (in seconds) for the whole inference session')
    adv_group.add_argument('--step_timeout', type=float, required=False, default=5 * 60,
                        help="Timeout (in seconds) for waiting the next step's inputs inside an inference session")
    adv_group.add_argument('--stats_report_interval', type=int, required=False,
                        help='Interval between two reports of batch processing performance statistics')
    adv_group.add_argument('--custom_module_path', type=str, required=False,
                        help='Path of a file with custom nn.modules, wrapped into special decorator')
    adv_group.add_argument("--balance_quality", type=float, default=0.75,
    ...
    adv_group.add_argument("--mean_balance_check_period", type=float, default=60,
