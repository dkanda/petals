...
def main():
    ...
    basic_group.add_argument("--token", type=str, nargs='?', default=None, const=True,
                        help="Hugging Face hub auth token for .from_pretrained(). "
                             "If the flag is passed with no value, it will load the token from the cache. "
                             "Example: --token, --token MY_TOKEN")
