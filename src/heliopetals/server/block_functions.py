...
from heliopetals.data_structures import Handle, InferenceMetadata
from heliopetals.server.backend import TransformerBackend
from heliopetals.server.task_pool import PrioritizedTaskPool
from heliopetals.server.task_prioritizer import TaskPrioritizerBase
from heliopetals.utils.convert_block import QuantType
from heliopetals.utils.misc import DUMMY, is_dummy
from heliopetals.utils.packaging import unpack_args_kwargs
...
async def run_rpc_forward(
    *flat_tensors: torch.Tensor,
    ...
    for backend, prompt in zip(requested_backends, prompts):
        ...
        assert isinstance(backend.inference_pool, PrioritizedTaskPool), "heliopetals support only prioritized pools"
        ...
        if hidden_states.numel() > 0:
            ...
            if peer_id:
                with open("token_log.txt", "a") as f:
                    f.write(f"Peer: {peer_id}, Tokens: {num_tokens}\n")
