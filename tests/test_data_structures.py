...
def test_server_info_from_tuple():
    ...
    with pytest.raises(ValueError, match=r"expected a tuple of length 2 or 3, but got 1"):
        ServerInfo.from_tuple((ServerState.ONLINE.value,))

    with pytest.raises(ValueError, match=r"expected a tuple of length 2 or 3, but got 4"):
