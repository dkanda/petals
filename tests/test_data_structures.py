...
def test_server_info_from_tuple():
    ...
    with pytest.raises(ValueError, match=r"info must have 3 elements, but got 1"):
        ServerInfo.from_tuple((ServerState.ONLINE.value,))

    with pytest.raises(ValueError, match=r"info must have 3 elements, but got 2"):
        ServerInfo.from_tuple((ServerState.ONLINE.value, 10.0))

    with pytest.raises(ValueError, match=r"info must have 3 elements, but got 4"):
