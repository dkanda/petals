...
def test_server_info_from_tuple():
    ...
    with pytest.raises(ValueError, match=r"expects a tuple of length 3, but got 2"):
        ServerInfo.from_tuple((ServerState.ONLINE.value, 10.0))

    with pytest.raises(ValueError, match=r"expects a tuple of length 3, but got 4"):
        ServerInfo.from_tuple((ServerState.ONLINE.value, 10.0, {}, "something extra"))
